...
- Ordering is a hard constraint, not a rough guess: assign "order" 1-based strictly by pedagogical prerequisite sequence - a unit (or a lesson inside a unit) must never precede another unit/lesson whose content it actually depends on. Before assigning numbers, use every file's topicSummary to work out which topics are foundational (assumed as background elsewhere) and which build on top of another topic - then order foundational topics first. If unit A's topicSummary reveals it builds on, extends, or applies a concept that unit B's topicSummary introduces from scratch, unit B must be ordered before unit A, even if unit A's files otherwise look earlier alphabetically, chronologically, or by folder. Do not fall back to input-list order or folder order except as a last resort when two topics are genuinely independent (no prerequisite relationship either way) - in that case list order is fine as a tiebreaker.
- Before finalizing, re-check your own proposed ordering pairwise for the whole unit list: for every unit, confirm no unit ordered after it is actually a prerequisite that unit's lessons depend on. Fix any such inversion by moving the prerequisite unit earlier rather than leaving it out of order.
